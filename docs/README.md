...
- 视图切换（3D 球面 / 2D 平面，V 键）：2D 用 Cesium 的 SCENE2D（等经纬度投影）。2D 下地形被压平，深度缓冲量不出水深，水体着色器另走一条路：每帧按正交相机算出「屏幕 → 经纬度」的仿射映射传进着色器，逐像素查高程图得到水深，视线垂直向下，穿水路程就等于水深。着色部分（吸收、散射、波浪、高光、日照）与 3D 共用同一个函数。实测映射与 Cesium 自己拾取的经纬度一致，陆地像素不受影响。切换不播 Cesium 的变形动画：变形途中既不是球面也不是平面，水体没法算。2D 下垂直夸张滑块置灰。
